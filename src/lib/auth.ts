...
export const auth = betterAuth({
  ...
  session: {
    ...
    updateAge: 60 * 60 * 24,
    /* cache the session in an HMAC-signed cookie (signed with BETTER_AUTH_SECRET
     * → can't be forged/tampered; a bad signature falls back to a DB lookup).
     * Short maxAge keeps revocation staleness tiny. */
